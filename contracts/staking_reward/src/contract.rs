...
use cosmwasm_std::{
    to_binary, Binary, Decimal, Deps, DepsMut, Env, MessageInfo, Response, StdResult, Uint128, Addr, SubMsg, CosmosMsg, WasmMsg, Reply, attr,
};

use cdp::staking_reward::{
...
fn update_config(
    ...
    if let Some(reward_contract) = reward_contract {
        config.control_contract = deps.api.addr_canonicalize(reward_contract.as_str())?;
    }
    ...
    Ok(Response::new()
        .add_submessage(SubMsg::reply_on_success(
            CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: deps.api.addr_humanize(&config.reward_contract)?.to_string(),
