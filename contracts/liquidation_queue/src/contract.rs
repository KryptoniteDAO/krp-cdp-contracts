...
use cdp::liquidation_queue::{Cw20HookMsg, ExecuteMsg, InstantiateMsg, QueryMsg, MigrateMsg};
...
pub fn whitelist_collateral(
    ...
    // check if the collateral is whitelisted in central_control
    let control_contract = deps.api.addr_humanize(&&config.control_contract)?;
    query_collateral_whitelist_info(
        deps.as_ref(),
    ...
    .map_err(|_| {
        ContractError::Std(StdError::generic_err(
            "This collateral is not whitelisted in Kryptonite control",
        ))
    })?;
...
pub fn update_collateral_info(
    ...
    if deps.api.addr_canonicalize(info.sender.as_str())? != config.owner {
        return Err(ContractError::Unauthorized(
            "update_collateral_info".to_string(),
            info.sender.to_string(),
        ));
    }
